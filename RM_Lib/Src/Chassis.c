     ...
     static uint16_t Speed,tim;		 
	   static uint16_t Ramp_rotate_ref;	
		 ...
		 switch(CHASSIS.Action){
			...
			/* µ×ÅÌ¸úËæ£¨Ö±½ÓÅÐ¶ÏYawÖá»úÐµ½Ç¶ÈÅÐ¶Ï×î½ü¹éÖÐÎ»ÖÃ£© */
...
void Chassis_Offset(){
    ...
    chassis_offset = (Gimbal_Motor[YAW].MchanicalAngle - Yaw_Mid_Front) / 1303.64f;   // µ×ÅÌ²¹³¥½Ç
    ...
    /* µ×ÅÌ²¹³¥(Ð¡ÍÓÂÝÄ£Ê½Ò²ÄÜÕý³£ÒÆ¶¯)YÖáÔÚÏß²ÅÄÜ½âËã²¹³¥ */
    if(DeviceState.Gimbal_State[YAW] == Device_Online){
        Communication_Speed_Tx.Chassis_Speed.forward_back_ref = forward_back_ref * arm_sin_f32( -chassis_offset)//µ×ÅÌ½âËã(Ð¡ÍÓÂÝÄ£Ê½Ò²ÄÜÕý³£ÒÆ¶¯)
