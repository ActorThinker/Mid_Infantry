...
typedef enum pid_Improvement_e {
    ...
    Derivative_On_Measurement = 0x02,   // 微分先行（对测量值微分）
    Trapezoid_Intergral = 0x04,         // 梯形积分
    Proportional_On_Measurement = 0x08, // 比例测量（对测量值比例）
    OutputFilter = 0x10,                // 输出滤波
    ChangingIntegralRate = 0x20,        // 变积分增益
    DerivativeFilter = 0x40,            // 微分项滤波
...
float PID_Count(PID_TypeDef *pid,float err);
/**
 * @brief 标准位置式PID参数
 */
typedef struct {
